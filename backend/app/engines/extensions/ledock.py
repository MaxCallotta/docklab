...
@register_engine
class LeDockEngine(BaseDockEngine):
    """LeDock 对接适配器（预留）。"""

    engine_id = "ledock"
    engine_name = "LeDock"
    description = "LeDock 快速对接（预留模板）"
    ...
    def preprocess_receptor(self, receptor_path: Path, work_dir: Path) -> Path:
        raise EngineNotImplementedError("LeDock 受体准备将在适配器落地时实现。")

    def preprocess_ligand(self, ligand_path: Path, work_dir: Path) -> Path:
        raise EngineNotImplementedError("LeDock 配体准备将在适配器落地时实现。")

    def run_dock(self, receptor_pdbqt: Path, ligand_pdbqt: Path, work_dir: Path) -> Path:
        raise EngineNotImplementedError("LeDock 执行逻辑为预留模板，尚未实现。")

    def parse_result(self, output_path: Path, log_path: Path | None = None) -> DockResult:
        raise EngineNotImplementedError("LeDock 结果解析为预留模板，尚未实现。")
