<template>
  <div class="page-shell">
    <h2 class="page-title">{{ $t('软件配置管理') }}</h2>
    <div class="settings-grid">
      <section>
        <SectionPanel :title="$t('本地软件路径')">
          <el-form label-width="150px">
            <el-form-item label="AutoDock Vina">
              <el-input v-model="form.vina_bin" :placeholder="$t('完整路径')" />
            </el-form-item>
            <el-form-item label="AutoDock 4">
              <el-input v-model="form.autodock4_bin" :placeholder="`autodock4.exe（${$t('预留')}）`" />
            </el-form-item>
            <el-form-item label="AutoGrid 4">
              <el-input v-model="form.autogrid4_bin" :placeholder="`autogrid4.exe（${$t('预留')}）`" />
            </el-form-item>
            <el-form-item label="PyMOL">
              <el-input v-model="form.pymol_bin" :placeholder="$t('完整路径')" />
            </el-form-item>
            <el-form-item label="OpenBabel">
              <el-input v-model="form.obabel_bin" :placeholder="$t('完整路径')" />
            </el-form-item>
          </el-form>

          <el-divider content-position="left">{{ $t('预留对接软件路径') }}</el-divider>
          ...
          <div v-for="engine in form.extra_engines" :key="engine.engine_id" class="extra-engine">
            <el-input v-model="engine.path" :placeholder="`${engine.engine_name} ${$t('可执行路径')}`" />
          </div>
        </SectionPanel>
      </section>

      <section>
        <SectionPanel :title="$t('全局预处理默认值')">
          <el-form label-width="150px">
            <el-form-item :label="$t('加氢')">
              <el-switch v-model="form.add_hydrogens" :active-text="$t('自动补氢')" />
            </el-form-item>
            <el-form-item :label="$t('电荷计算方法')">
        ...
        <div class="save-bar">
          <el-button type="primary" :loading="saving" @click="saveAll">{{ $t('保存全部配置') }}</el-button>
        </div>
      </section>
    </div>

    <SectionPanel :title="$t('参数模板管理')">
      <div class="template-toolbar">
        <el-button type="primary" plain @click="templateDialog = true">{{ $t('新增模板') }}</el-button>
      </div>
      <el-table :data="settings.templates" :empty-text="$t('暂无模板')">
        <el-table-column prop="name" :label="$t('模板名称')" min-width="200" />
        <el-table-column :label="$t('参数摘要')" min-width="320" show-overflow-tooltip>
          <template #default="{ row }">
        ...
        <el-table-column :label="$t('操作')" width="180" fixed="right">
          <template #default="{ row }">
            <el-button size="small" link type="primary" @click="loadTemplate(row)">{{ $t('加载') }}</el-button>
            <el-button size="small" link type="danger" @click="removeTemplate(row)">{{ $t('删除') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    </SectionPanel>

    <SectionPanel :title="$t('本地日志')">
      <div class="log-toolbar">
        <el-date-picker
          ...
          style="width: 160px"
        />
        <el-select v-model="logLevel" clearable :placeholder="$t('全部级别')" style="width: 130px">
          <el-option label="INFO" value="INFO" />
          <el-option label="WARNING" value="WARNING" />
          <el-option label="ERROR" value="ERROR" />
        </el-select>
        <el-button type="primary" plain :loading="logLoading" @click="loadLogs">{{ $t('刷新日志') }}</el-button>
      </div>
      <el-table :data="logEntries" size="small" max-height="460" :empty-text="$t('暂无日志')">
        <el-table-column prop="time" :label="$t('时间')" width="165" />
        <el-table-column :label="$t('级别')" width="90">
          <template #default="{ row }">
            <el-tag :type="logTagType(row.level)" size="small" effect="plain">{{ row.level }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="message" :label="$t('消息')" min-width="300" show-overflow-tooltip />
        <el-table-column prop="task_id" :label="$t('任务 ID')" width="120" show-overflow-tooltip />
        <el-table-column prop="command" :label="$t('执行命令')" min-width="240" show-overflow-tooltip />
        <el-table-column prop="source_file" :label="$t('输入文件')" width="160" show-overflow-tooltip />
        <el-table-column prop="error_stack" :label="$t('错误堆栈')" min-width="260" show-overflow-tooltip />
      </el-table>
    </SectionPanel>

    <el-dialog v-model="templateDialog" :title="$t('保存参数模板')" width="440px">
      <el-form label-width="90px">
        <el-form-item :label="$t('模板名称')" required>
          <el-input v-model="templateName" :placeholder="$t('例如：ATP 口袋-默认盒子')" />
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="saveTemplateFromJson">{{ $t('保存') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
async function saveAll() {
  ...
  try {
    ...
    ElMessage.success(t('配置已保存，重启后端服务后生效'))
  } finally {
    saving.value = false
  }
}
...
function loadTemplate(row) {
  ...
  ElMessage.success(t('参数模板已加载，正在前往新建任务页'))
  router.push('/')
}

async function removeTemplate(row) {
  await ElMessageBox.confirm(t('删除模板「{name}」？', { name: row.name }), t('删除模板'), { type: 'warning' })
