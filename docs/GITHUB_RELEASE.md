...
git branch -M main
git remote add origin https://github.com/MCXDL/docklab.git
...
4. 上传 exe 附件：`dist\CaddPlatform.exe`
5. 填写发布说明后发布
...
```text
https://github.com/MCXDL/docklab/releases/latest/download/CaddPlatform.exe
```
...
- 本地打包时如需内置外部引擎，请把工具目录放入 `tools/`，或设置 `OPENBABEL_DIR`、`VINA_DIR`、`AUTODOCK_TOOLS_DIR`
- `CaddPlatform.exe` 没有数字签名，其他用户首次运行可能被 SmartScreen 或杀毒软件拦截，需要选择“仍要运行”
- 当前 exe 仅支持 Windows 10/11 64 位
- exe 默认使用 `%LOCALAPPDATA%\CaddPlatform\data` 作为数据目录，可通过环境变量 `PAX_DATA_ROOT` 覆盖
